...
public class Ec2InstanceRole extends Stack {
    ...
    public Ec2InstanceRole(final Construct scope, final String id, final StackProps props) {
        ...
        instanceRole = new Role(this, "WebAppInstanceRole",
                RoleProps.builder().assumedBy(ServicePrincipal.Builder.create("ec2.amazonaws.com").build()).build());

        PolicyStatement getParameters = PolicyStatement.Builder.create().effect(Effect.ALLOW)
                .actions(Arrays.asList("ssm:GetParameters"))
                .resources(
                        Arrays.asList("arn:aws:ssm:" + env.getRegion() + ":" + env.getAccount() + ":parameter/duo_*"))

                .build();
        PolicyStatement getParameter = PolicyStatement.Builder.create().effect(Effect.ALLOW)
                .actions(Arrays.asList("ssm:GetParameter"))
                .resources(
                        Arrays.asList("arn:aws:ssm:" + env.getRegion() + ":" + env.getAccount() + ":parameter/duo_*"))

                .build();
        PolicyStatement decrypt = PolicyStatement.Builder.create().effect(Effect.ALLOW)
